...
import logging
import pickle
from src.models.etm import ETM
...
class ETMTrainer:
    def __init__(self,
        ...
        self.model = ETM(num_topics=num_topics,
    ...
    def run(self):
        ...
        for epoch in range(self.max_epochs):
            ...
            logger.info(
                f'Training epoch {epoch} completed; training loss: {self.logger["train/kld"][-1] + self.logger["train/rec"][-1]}')
            self.model.eval()
            self.test_one_epoch()
            logger.info(
                f'Validation epoch {epoch} completed; validation loss: {self.logger["val/kld"][-1] + self.logger["val/rec"][-1]}')
        ...
        # load best model
        self.load_model()
        topic_embeddings = self.model.topic_embeddings.weight.data.cpu().numpy().T  # (K, E)
        word_embeddings = self.model.word_embeddings.weight.data.cpu().numpy().T  # (E, V)
        beta = topic_embeddings @ word_embeddings  # (K, V)
        ...
        logger.info(
            f"Evaluation completed. Coherence scores are: {coherence}, topic diversity: {topic_diversity}")
        ...
        with open(os.path.join(self.save_path, 'topics.txt'), 'w') as f:
            ...
            f.write(
                f"Coherence scores are: coherence: {coherence}, topic diversity: {topic_diversity}")
    ...
    def train_one_epoch(self):
        ...
        for batch in self.train_dl:
            ...
            prior_mean = torch.zeros_like(dist.loc).to(self.device)
            prior_var = torch.ones_like(dist.scale).to(self.device)
            prior = torch.distributions.LogNormal(prior_mean, prior_var)
